...
export default function ManagerCheck(
	request, response,
	database, databaseKey,
	processId, processType){

	let accountSchema = request.body
	if(!SchemaCheck(response, accountSchema, ['id','peerId','channelId','managerId'], `${processType} 매니저 확인`, processId)) return
	...
	Authorize(
		...
		// 데이터베이스에 정보입력을 시도합니다.
		database[databaseKey].isManager(
			accountSchema.channelId,
			accountSchema.managerId,
