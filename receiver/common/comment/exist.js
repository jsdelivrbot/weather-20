...
export default function Exist(
	...
	if(!SchemaCheck(response, accountSchema, ['documentId', 'commentNum'], `${processType} 존재여부 확인`, processId)) return

	// 데이터베이스에 정보 존재여부 확인을 시도합니다.
	database[databaseKey].exist(accountSchema.documentId, accountSchema.commentNum, (isExist)=>{
