...
export default function Exist(
	...
	if(!SchemaCheck(response, accountSchema, ['documentId', 'commentNum'], `${processType} 읽어오기`, processId)) return

	// 데이터베이스에서 정보 읽어오기를 시도합니다.
	database[databaseKey].read(accountSchema.documentId, accountSchema.commentNum, (isExist, documentSchema)=>{
