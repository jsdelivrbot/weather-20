...
export function UpdateAddressBar(){
	...
	if(window.armyWeather.private.address.sub.length == 0){
		...
		<div class="swiper-slide">
			<div id="location-item-1" class="location-small-box location-small-box-gps ${gpsStyleOption} location-small-box-cover">
	...
	for(let subIndex in window.armyWeather.private.address.sub){
		...
		innerHTML += `
		<div class="swiper-slide">
			<div id="location-item-${realIndex}" class="location-small-box ${styleOption} location-small-box-cover">
	...
	if(subCount < 4){
		...
		for(let i=1;i<=needToAddCount;i++){
			...
			innerHTML += `<div class="swiper-slide">
				<div id="location-item-${realIndex}" class="location-small-box location-small-box-yet ${styleOption} location-small-box-cover">
	...
	addressSlider = new Swiper('.local-address-slider', {
		...
		//drinkGraphic.addEventListener('long-press', drinkDelInteract)
			touchEnd: (ev)=>{
				if(isTouchMoved < 4){
					//alert(`터치확인 ${isTouchMoved}`)
					//alert(ev.path)
					//alert(JSON.stringify(ev, null, 2))
					let slideElement = ev.path[0]
					if(typeof slideElement['id'] == 'undefined'
						|| slideElement.id.indexOf('location-item-') == -1){

						//alert(`정의되지 않은 주소 터치 ${slideElement.id}`)
						return;
					}
