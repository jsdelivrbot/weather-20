...
export function usage(){
	Logger.log(`/channel-login <계정ID> <계정PW>`, `[Frontend:Channel]`)
}
...
export default (commandArgs) => {
	...
	Logger.log(`서버에 채널접속 요청을 전송합니다.`, `[Frontend:Channel]`)

	API.call('/api/channel/login',{
		id: commandArgs[0],
		...
		// 기본 처리콜백
		DefaultCallback(paramData, '채널접속', `Channel`)
	})
}